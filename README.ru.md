...
**Переключение прокси в один клик для Chrome, Firefox и Zen Browser.**
...
- **Одна большая кнопка** — включает и выключает активный прокси; иконка на панели становится зелёной или серой.
...
- **Лёгкое** — одна кодовая база (WXT + Svelte 5), ~90 КБ в распакованном виде, ноль рантайм-зависимостей.

## Установка

В магазины пока не опубликовано — сборка из исходников:
...
- **Пинг — эвристика.** Расширениям недоступны сырые TCP-сокеты, поэтому задержка меряется
  временем ответа порта на `fetch`. Для HTTP-прокси точно; для SOCKS быстрый сброс соединения
  неотличим от открытого, но не-HTTP порта.
...
Ишьюс и пулл-реквесты приветствуются — см. [CONTRIBUTING.md](CONTRIBUTING.md).
