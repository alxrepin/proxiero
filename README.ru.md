...
- **Раздельное туннелирование** — пропускайте через прокси только выбранные домены (белый список) или всё, кроме них (чёрный список). Домен совпадает вместе со всеми поддоменами.
...
| Раздельное туннелирование | сгенерированный `pac_script`, когда задан список доменов | проверка по хосту в `proxy.onRequest` |
...
utils/                   # чистые модули: типы, storage, пинг, парсинг, i18n, раздельное туннелирование
